...
config = None
logger = None
# veeamUsername = os.getenv('VEEAM_EM_USERNAME')             ## read-only username. if using domain based account, add it like user@domain.com
# veeamPassword = os.getenv('VEEAM_EM_PASSWORD')             ## read-only password
# veeamEmRestServer = os.getenv('VEEAM_EM_HOST')               ## hostname where veeam em is running
# veeamEmRestPort = os.getenv('VEEAM_EM_PORT')                 ## port on which veeam em rest api is listening
# veeamEmRestProtocol = os.getenv('VEEAM_EM_PROTOCOL')         ## http or https
# veeamEmSkipVerifySsl = os.getenv('VEEAM_EM_SKIP_VERIFY_SSL') ## skip verify of the tls certificate - true or false
# veeamEmTimeout = os.getenv('VEEAM_EM_TIMEOUT')               ## how long before request times out - in seconds
...
#      logger.debug( 'metrics are: {0}'.format(metrics) )
   ...
   apis = []
   if 'VEEAM_EM_HOSTNAME' in os.environ:
      host = os.environ['VEEAM_EM_HOSTNAME']
      user = os.environ['VEEAM_EM_USERNAME']
      password = os.environ['VEEAM_EM_PASSWORD']
      protocol = "https"
      verify_ssl = False
      timeout = 10.0
      keep_session = True
      ...
      labels = None
      proxy = None
      print(f"Using VEEAM_EM_HOSTNAME from environment variable: {host}")
   else:
      for veeam in config['veeams']:
         ...
         if 'VEEAM_EM_USERNAME' in os.environ:
            user = os.environ['VEEAM_EM_USERNAME']
         elif 'user' in veeam:
            user = veeam['user']

         if 'VEEAM_EM_PASSWORD' in os.environ:
            password = os.environ['VEEAM_EM_PASSWORD']
         elif 'password' in veeam:
            password = veeam['password']
